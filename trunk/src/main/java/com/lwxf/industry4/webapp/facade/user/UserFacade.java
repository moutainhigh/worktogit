...
public interface UserFacade extends BaseFacade {
    ...
    RequestResult toFactoryLogin(MapContext userMap, HttpServletRequest request);
	...
	LoginedUser findLoginedUserById(String id);
	...
	RequestResult updateUserMessage(String userId, MapContext mapContext);

	RequestResult findUserInfo(String uid);

	RequestResult updatePassword(String userId,MapContext mapContext);//以后调整

	RequestResult registerAuthCode(MapContext mapContext);

	RequestResult BuserRegister(MapContext mapContext);

	RequestResult restPassword(String userId);

	/** 激活工厂账户启用或禁用 */
	RequestResult enableAndProhibitFactory(String uid);

	/** 激活工厂账户登录微信小程序启用或禁用 */
	RequestResult enableAndProhibitWxLogin(String uid);

	/** 激活工厂账户登录PC启用或禁用 */
	RequestResult enableAndProhibitPcLogin(String uid);
}
