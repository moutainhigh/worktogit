package com.lwxf.industry4.webapp.web.filter;
...
public class LwxfFirstFilter extends OncePerRequestFilter {
	...
	// 用于处理当前公司id
	private Pattern mallAndAdminPathPattern = Pattern.compile("/(bdmin|fadmin)");
	@Override
...
//		}
		...
		try {
			if (!AppBeanInjector.configuration.isOnProd()) {
				...
				} else if(LwxfStringUtils.isEquals(debugClient, "mobile", false)){
					LwxfUserAgent agent = LwxfUserAgent.parseUserAgentString("MobileMessenger");
					httpSession.setAttribute("user_agent", agent);
			...
			// 处理当前公司
			if(this.mallAndAdminPathPattern.matcher(sPath).matches()){
				String cid = request.getParameter(WebConstant.REQUEST_PARAMETER_KEY_CID);
				if(StringUtils.isNotBlank(cid)){
					WebUtils.setCurrCompanyId(cid);
				}
			}
