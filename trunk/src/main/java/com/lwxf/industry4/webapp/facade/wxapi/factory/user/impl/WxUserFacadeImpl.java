...
import com.lwxf.industry4.webapp.common.utils.PasswordUtil;
import com.lwxf.industry4.webapp.common.utils.StringUtil;
...
public class WxUserFacadeImpl extends BaseFacadeImpl implements WxUserFacade {
	...
	public RequestResult userSmsCheck(MapContext userMap) {
	...
	public RequestResult userLogin(MapContext userMap) {
		...
		//加密单点登录信息
		String randomNumberCode = StringUtil.getRandomNumberCode(16);
		//redis保存randomNumberCode在另外一个系统使用randomNumberCode解密出用户名并进行登录操作
		String ssoToken = PasswordUtil.encrypt(loginName, randomNumberCode, PBESALT);
		//SSOToken 使用登录字段
		redisUtils.set(ssoToken,randomNumberCode);
	...
	public RequestResult updateUserInfo(String userId,MapContext mapContext) {
		...
		if(mapContext.containsKey("avatar")){
			String avatar=mapContext.getTypedValue("avatar",String.class);
			mapContext1.put("avatar",avatar);
		}
