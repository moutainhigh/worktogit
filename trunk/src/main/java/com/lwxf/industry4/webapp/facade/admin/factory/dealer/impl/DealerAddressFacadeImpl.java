 ...
 *
 * @author：DJL
 * @create：2019/12/2 10:19
 * @version：2019 Version：1.0
 * @company：老屋新房 Created with IntelliJ IDEA
 */
@Component(value = "dealerAddressFacade")
public class DealerAddressFacadeImpl extends BaseFacadeImpl implements DealerAddressFacade {
...
//        //如果仅剩一条收货地址，则自动改为默认地址
       List<DealerAddress> addressList= this.dealerAddressService.findListByCid(address.getCompanyId());
       if(addressList!=null&&addressList.size()==1){
           DealerAddress dealerAddress=addressList.get(0);
           if(dealerAddress.getChecked()!=1) {//非默认地址
               MapContext mapContext=MapContext.newOne();
               mapContext.put("id",dealerAddress.getId());
               mapContext.put("checked",1);
               this.dealerAddressService.updateByMapContext(mapContext);
           }
       }
        return ResultFactory.generateSuccessResult();
