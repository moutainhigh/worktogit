...
import com.lwxf.industry4.webapp.facade.admin.factory.statement.DispatchBillStatementFacade;
 ...
 */
@Component("pcDispatchBillStatementFacade")
public class DispatchBillStatementFacadeImpl extends BaseFacadeImpl implements DispatchBillStatementFacade {
	...
	public RequestResult dispatchBillStatements(String branchId, String countType, MapContext mapContext) {
		MapContext result=MapContext.newOne();
		String dateType = mapContext.getTypedValue("dateType", String.class);//要查询的方式 （按日、月、年）
		//柱状图数据
		mapContext.put("branchId",branchId);
		mapContext.put("countType",countType);
		//选择开始时间和结束时间
		...
		//圆饼图信息(按日查询不用处理)
		if(dateType.equals("1")){//按月查询，开始时间为开始月的第一天，结束时间为结束月的最后一天
			startTime=DateUtilsExt.getFirstDayOfMonth(startTime);
			endTime=DateUtilsExt.getLastDayOfMonth(endTime);
		}
		if(dateType.equals("2")){//按年查询，开始时间为开始年的第一天，结束时间为结束年的最后一天
	...
	@Override
	public RequestResult dispatchBillStatementsNow(String branchId) {
		MapContext result=MapContext.newOne();
		//待发货数
		Integer nowToBeDeliver=this.dispatchBillStatementService.dispatchBillStatementsNow(branchId);
		result.put("nowToBeDeliver",nowToBeDeliver);
		return ResultFactory.generateRequestResult(result);
	}

	@Override
	public RequestResult dispatchBillStatementsTrend(String branchId, String startTime, String endTime,String dateType) {
		...
		if(dateType.equals("1")) {
			...
			try {
				dates = DateUtilsExt.findDates(format.parse(startTime),format.parse(endTime));
			} catch (ParseException e) {
				e.printStackTrace();
			}
		}else {//本年
			...
			try {
				dates = DateUtilsExt.getMonthBetween(dateFormat.parse(startTime),dateFormat.parse(endTime));
			} catch (ParseException e) {
				e.printStackTrace();
			}
		}
		for(Date date:dates){
			MapContext mapContext=MapContext.newOne();
			Integer count=this.dispatchBillStatementService.dispatchBillStatementsTrend(branchId,date,dateType);
			String dateTime = dateFormat.format(date);
			mapContext.put("count",count);
			mapContext.put("format",dateTime);
	...
	@Override
	public RequestResult dispatchBillStatementsRank(String branchId, String startTime, String endTime) {
		MapContext mapContext=MapContext.newOne();
		mapContext.put("branchId",branchId);
		mapContext.put("startTime",startTime);
		mapContext.put("endTime",endTime);
		List<MapContext> result=this.dispatchBillStatementService.dispatchBillStatementsRank(mapContext);
		return ResultFactory.generateRequestResult(result);
	}
