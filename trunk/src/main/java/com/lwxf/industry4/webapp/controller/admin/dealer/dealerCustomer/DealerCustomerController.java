package com.lwxf.industry4.webapp.controller.admin.dealer.dealerCustomer;
 ...
 */
@Api(value = "DealerCustomerController", tags = {"B端后台管理接口:终端客户管理"})
@RestController(value = "DealerCustomerController")
@RequestMapping(value = "/api/b/customers", produces = WebConstant.RESPONSE_CONTENT_TYPE_JSON_CHARTSET)
public class DealerCustomerController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "type",value = "列表内：客户类型 1-意向客户 2-成交客户 3-执行客户",dataType = "String",paramType = "query")
    })
     ...
     * @return
     */
    @PostMapping("/addWxCustomers2")
    @ApiOperation(value = "添加客户",notes = "添加客户接口")
    public RequestResult addWxCustomers2(@RequestBody MapContext mapContext){
        String uid = WebUtils.getCurrUserId();
        if (uid == null) {
        ...
        CompanyCustomer customer = this.companyCustomerService.findByPhoneAndBranchId(mobile, cid);
        if(customer!=null){
    ...
    public RequestResult updateCustomer(@PathVariable String customerId,
    ...
    public RequestResult deleteCustomer(@PathVariable String customerId) {
        ...
        return this.bWxCustomerFacade.deleteWxCustomer(customerId, cid);
