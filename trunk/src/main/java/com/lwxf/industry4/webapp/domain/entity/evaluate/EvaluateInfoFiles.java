...
/**
 * 功能：evaluate_info_files 实体类
 *
 * @author：lyh
 * @created：2019-11-30 02:04 
 * @version：2018 Version：1.0 
 * @dept：老屋新房 Created with IntelliJ IDEA 
 */ 
@ApiModel(value="evaluate_info_files对象", description="evaluate_info_files")
@Table(name = "evaluate_info_files",displayName = "evaluate_info_files")
public class EvaluateInfoFiles extends IdEntity  {
	...
	@Column(type = Types.CHAR,length = 13,name = "evaluate_info_id",displayName = "访问id")
	@ApiModelProperty(value = "访问id")
	private String evaluateInfoId;
	@Column(type = Types.TINYINT,name = "category",displayName = "分类：0 - 现场照片 1 - 合影")
	@ApiModelProperty(value = "分类：0 - 现场照片 1 - 合影")
	private Byte category;

    public EvaluateInfoFiles() {  
     } 
	...
	public RequestResult validateFields() {
		...
		if (LwxfStringUtils.getStringLength(this.evaluateInfoId) > 13) {
			validResult.put("evaluateInfoId", ErrorCodes.VALIDATE_LENGTH_TOO_LONG);
		}
	...
	private final static List<String> propertiesList = Arrays.asList("id","path","fullPath","mime","name","originalMime","status","creator","created","evaluateInfoId","category");
	...
	public static RequestResult validateFields(MapContext map) {
		...
		if(map.containsKey("evaluateInfoId")) {
			if (LwxfStringUtils.getStringLength(map.getTypedValue("evaluateInfoId",String.class)) > 13) {
				validResult.put("evaluateInfoId", ErrorCodes.VALIDATE_LENGTH_TOO_LONG);
			}
		}
		if(validResult.size()>0){
